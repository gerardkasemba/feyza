...
/** Loan shape required for HTML contract generation */
export interface ContractLoan {
  ...
  borrower?: User | null;
  lender?: User | null;
  business_lender?: (BusinessProfile & { owner?: User }) | null;
  schedule?: PaymentScheduleItem[];
}

export function generateContractHtml(loan: ContractLoan): string {
  const borrower = loan.borrower;
  const borrowerDisplayName =
    borrower?.full_name || loan.borrower_name || 'Borrower';
  const borrowerDisplayEmail = borrower?.email ?? '—';

  const lenderName =
    loan.lender_type === 'business'
      ? loan.business_lender?.business_name
      : loan.lender_name || loan.lender?.full_name || loan.invite_email || 'Pending';
  const lenderEmail =
    loan.lender_type === 'business'
      ? loan.business_lender?.contact_email
      : loan.lender_email || loan.lender?.email || loan.invite_email || '—';
  ...
  const lastPayment = (loan.schedule || [])
    .slice()
    .sort((a: PaymentScheduleItem, b: PaymentScheduleItem) => new Date(b.due_date).getTime() - new Date(a.due_date).getTime())[0];

  const endDate = lastPayment?.due_date ? format(new Date(lastPayment.due_date), 'MMMM d, yyyy') : 'TBD';

  const currency = loan.currency || 'USD';
  const money = (n: unknown) => (typeof n === 'number' ? n.toLocaleString() : '0');
  ...
  const payScheduleRows =
    (loan.schedule || [])
      .slice()
      .sort((a: PaymentScheduleItem, b: PaymentScheduleItem) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime())
      .map((p: PaymentScheduleItem, idx: number) => {
...
<body>
  <div class="page">
        ...
        <div class="bd">
          <div class="parties">
            <div class="party">
              ...
              <p class="name">${escapeHtml(borrowerDisplayName)}</p>
              <p class="kv">
                <span><b>Email:</b> ${escapeHtml(borrowerDisplayEmail)}</span>
            ...
            <div class="party">
              ...
              <p class="kv">
                ${lenderEmail ? `<span><b>Email:</b> ${escapeHtml(lenderEmail)}</span>` : ''}
                ${
      ...
      <div class="card">
        ...
        <div class="bd">
          <div class="clauses">
            ...
            <div class="clause">
              <h3><span class="badge-num">3</span> Payment Authorization</h3>
              <p>When auto-pay is enabled, the borrower authorizes Feyza to initiate ACH debits from the connected bank account on scheduled due dates. Otherwise, the borrower agrees to make payments by the due date via the agreed method (e.g. bank transfer, PayPal, Cash App, Venmo, or Zelle).</p>
            </div>
            <div class="clause">
      ...
      <div class="card">
        ...
        <div class="bd">
          <div class="clauses">
            ...
            <div class="clause">
              <h3><span class="badge-num">2</span> Disbursement</h3>
              <p>The lender agrees to disburse the loan amount to the borrower by the agreed method (e.g. ACH bank transfer, PayPal, Cash App, Venmo, or Zelle) once both parties have signed this agreement.</p>
            </div>
            <div class="clause">
    ...
    <div class="signwrap">
      <div class="sig">
        <div class="who">
          <b>${escapeHtml(borrowerDisplayName)}</b>
