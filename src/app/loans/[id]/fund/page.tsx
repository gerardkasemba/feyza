      ...
      <div className="min-h-screen flex flex-col bg-neutral-50">
        ...
        <main className="flex-1 flex items-center justify-center">
          <Card className="max-w-md text-center p-8">
            ...
            <h1 className="text-2xl font-bold text-neutral-900 mb-2">Funds Sent! 🎉</h1>
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Card className="mb-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-2xl font-bold text-neutral-900">Fund This Loan</h1>
                <p className="text-neutral-500">Review and sign the agreement, then send funds</p>
              </div>
            </div>
            ...
            <div className="bg-neutral-50 rounded-xl p-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-medium">{borrower?.full_name || 'Unknown'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{loan.interest_rate || 0}% ({loan.interest_type || 'simple'})</p>
                </div>
                <div>
                  <span className="text-neutral-500">Total Repayment</span>
                  <p className="font-medium">{formatCurrency(loan.total_amount || loan.amount, loan.currency)}</p>
                </div>
                <div>
                  <span className="text-neutral-500">Installments</span>
                  <p className="font-medium">{loan.total_installments} × {formatCurrency(loan.repayment_amount || 0, loan.currency)}</p>
                </div>
                <div>
                  <span className="text-neutral-500">First Payment</span>
                  <p className="font-medium">{loan.start_date ? formatDate(loan.start_date) : 'TBD'}</p>
                </div>
                {loan.purpose && (
            ...
            <div className="space-y-4 mb-6">
              ...
              {/* Your Bank */}
              <div className={`p-4 rounded-xl border ${lenderBankConnected ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${lenderBankConnected ? 'bg-green-100' : 'bg-red-100'}`}>
                    ...
                    <div>
                      ...
                      {lenderBankConnected ? (
                        <p className="font-medium">{lenderBankName || 'Bank Account'} {lenderBankMask && `(••••${lenderBankMask})`}</p>
                      ) : (
              ...
              {/* Borrower's Bank */}
              <div className={`p-4 rounded-xl border ${borrowerBankConnected ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${borrowerBankConnected ? 'bg-green-100' : 'bg-red-100'}`}>
                      <User className={`w-5 h-5 ${borrowerBankConnected ? 'text-green-600' : 'text-red-600'}`} />
                    </div>
                    <div>
                      <p className="text-sm text-neutral-500">To: {borrower?.full_name || 'Borrower'}'s Bank</p>
                      {borrowerBankConnected ? (
                        <p className="font-medium">{borrowerBankName || 'Bank Account'} {borrowerBankMask && `(••••${borrowerBankMask})`}</p>
                      ) : (
                        <p className="font-medium text-red-700">Not Connected</p>
                      )}
                    </div>
              ...
              {!borrowerBankConnected && (
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-amber-700 text-sm">
                    ⚠️ The borrower hasn't connected their bank account yet. Please ask them to connect their bank in Settings before you can fund this loan.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="border rounded-xl p-4 mb-6">
              ...
              <div className="text-sm text-neutral-600 space-y-2 max-h-48 overflow-y-auto mb-4 p-3 bg-neutral-50 rounded-lg">
                ...
                <p><strong>2. Disbursement</strong></p>
                <p>
