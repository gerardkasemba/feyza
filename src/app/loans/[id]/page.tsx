...
export default function LoanDetailPage() {
  ...
  // Live transfer status (kept from your code shape)
  ...
  /* -------------------------------------------
     Real-time updates (kept, simplified)
  -------------------------------------------- */
  useEffect(() => {
    if (!loanId) return;
    ...
    const loanChannel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'loans', filter: `id=eq.${loanId}` }, (payload) => {
        const n = payload.new as any;
        const o = payload.old as any;

        if (n.status !== o?.status) {
          if (n.status === 'active') showToast({ type: 'success', title: 'Loan Active!', message: 'The loan is now active' });
          if (n.status === 'completed') showToast({ type: 'success', title: '🎉 Loan Completed!', message: 'Fully repaid' });
          if (n.status === 'declined') showToast({ type: 'warning', title: 'Loan Declined', message: 'The request was declined' });
        }

        if (n.disbursement_status !== o?.disbursement_status) {
          if (n.disbursement_status === 'completed')
            showToast({ type: 'success', title: 'Funds Transferred!', message: 'Funds deposited successfully' });
          if (n.disbursement_status === 'processing')
            showToast({ type: 'info', title: 'Transfer Processing', message: 'ACH transfer in progress' });
        }
    ...
    const scheduleChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'payment_schedule', filter: `loan_id=eq.${loanId}` }, (payload) => {
        const n = payload.new as any;
        if (payload.eventType === 'UPDATE' && n?.is_paid && n?.status === 'confirmed') {
          showToast({ type: 'success', title: 'Payment Confirmed!', message: 'Payment has been confirmed' });
        }
        refetchSchedule();
        refetchLoan();
      })
    ...
    const transferChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'transfers', filter: `loan_id=eq.${loanId}` }, async (payload) => {
        const n = payload.new as any;
        if (n?.status === 'completed') showToast({ type: 'success', title: 'Transfer Complete!', message: 'Funds transferred' });
        if (n?.status === 'failed') showToast({ type: 'error', title: 'Transfer Failed', message: 'Please check details' });
  ...
  /* -------------------------------------------
     Transfer status (your mapping kept)
  -------------------------------------------- */
  const fetchTransferStatus = useCallback(async () => {
    ...
    try {
      ...
      if (transferStatusVal === 'processed' || transferStatusVal === 'completed') {
        status = 'completed';
        statusMessage = 'Funds have been deposited in the bank account';
      } else if (
  ...
  useEffect(() => {
    ...
    const shouldPoll =
      loan.status === 'active' &&
      (((loan as any).disbursement_status === 'processing') || ((loan as any).disbursement_status !== 'completed' && !loan.funds_sent));
  ...
  const handleAcceptLoan = async () => {
    ...
    try {
      ...
      if (response.ok && data.redirectUrl) {
        ...
        showToast({ type: 'success', title: 'Loan Accepted', message: 'Accepted successfully' });
        router.refresh();
      } else {
  ...
  const handleDeclineLoan = () => setShowDeclineDialog(true);
  ...
  const executeDeclineLoan = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast({ type: 'info', title: 'Loan Declined', message: 'The request has been declined' });
  ...
  const handleCancelLoan = () => setShowCancelDialog(true);
  ...
  const executeCancelLoan = async () => {
    ...
    try {
      ...
      if (response.ok) {
        showToast({ type: 'info', title: 'Request Cancelled', message: 'Your request has been cancelled' });
        router.push('/dashboard');
      } else {
        const data = await response.json();
  ...
  const handleProcessPayment = (paymentId: string) => {
    ...
    setPaymentConfirmDialog({ isOpen: true, paymentId, isBorrower: isBorrowerForPayment });
  };
    ...
    try {
      ...
      if (response.ok) {
        showToast({
          ...
          message: isBorrower ? 'ACH transfer started (1-3 business days).' : 'Payment processed successfully.',
        });
      ...
      showToast({ type: 'error', title: 'Error', message: e?.message || 'Failed to process payment' });
    } finally {
  ...
  const handleSendReminder = async (paymentId?: string) => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Reminder Sent', message: 'Reminder sent to borrower' });
      setShowReminderModal(false);
      setReminderMessage('');
      await refetchSchedule();
    } catch (error: any) {
      showToast({ type: 'error', title: 'Failed to Send', message: error.message || 'Please try again.' });
    } finally {
  ...
  const handleSendFunds = async () => {
    ...
    try {
      ...
      const { error: uploadError } = await supa.storage.from('loan-documents').upload(filePath, fundsProofFile);

      let proofUrl: string | null = null;
      if (!uploadError) {
        const { data } = supa.storage.from('loan-documents').getPublicUrl(filePath);
      ...
      if (response.ok) {
        ...
        showToast({ type: 'success', title: 'Funds Sent', message: 'Payment confirmation recorded' });
        await fetchTransferStatus();
      } else {
        const error = await response.json();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-neutral-50 dark:bg-neutral-950">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Card className="mb-6">
            <div className="flex flex-col gap-6">
              ...
              {/* Progress */}
              <div className="rounded-2xl bg-neutral-50 dark:bg-neutral-900/40 border border-neutral-200 dark:border-neutral-800 p-5">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-neutral-900 dark:text-white">Repayment progress</p>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">
                      {paidCount} of {schedule.length} payments completed • {Math.round(progress)}%
                    </p>
                  </div>
                  <div className="w-full md:w-80">
                    <div className="h-2 bg-neutral-200 dark:bg-neutral-800 rounded-full overflow-hidden">
                      <div className="h-full bg-green-500 dark:bg-green-400 rounded-full" style={{ width: `${progress}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 grid md:grid-cols-3 gap-3 text-sm">
                  <div className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-4">
                    <p className="text-neutral-500 dark:text-neutral-400">Repayment</p>
                  ...
                  <div className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-4">
                    ...
                    <p className="font-semibold text-neutral-900 dark:text-white">{formatDate(loan.start_date)}</p>
                  </div>

                  <div className="rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-4">
                    <p className="text-neutral-500 dark:text-neutral-400">Purpose</p>
                    <p className="font-semibold text-neutral-900 dark:text-white">{loan.purpose || '—'}</p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-amber-800/80 dark:text-amber-300/80">
                          {isBorrower
                            ? 'Your request was sent. You will be notified once the lender responds.'
                            : `A borrower requested ${formatCurrency(loan.amount, loan.currency)}.`}
                        </p>
                      </div>
                    </div>
          ...
          <div className="sticky top-[72px] z-10 mb-6">
            <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50/90 dark:bg-neutral-950/70 backdrop-blur px-3 py-2">
              <div className="flex items-center gap-1 overflow-x-auto no-scrollbar">
                ...
                {hasAgreementTab && (
                  <TabButton active={tab === 'agreement'} onClick={() => setTab('agreement')} icon={FileText} label="Agreement" />
                )}
                {hasRemindersTab && (
                    ...
                    {borrowerRatingData?.recommendation && (
                      <div className={`mt-3 p-3 rounded-lg text-sm ${
                        borrowerRatingData.rating?.overall === 'great' || borrowerRatingData.rating?.overall === 'good' 
                          ...
                          : borrowerRatingData.rating?.overall === 'poor' || borrowerRatingData.rating?.overall === 'bad' || borrowerRatingData.rating?.overall === 'worst'
                ...
                {schedule.filter((s) => !s.is_paid).slice(0, 5).map((p) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-neutral-500 dark:text-neutral-400">Due {formatDate(p.due_date)}</p>
                    </div>
                    <Button size="sm" variant="outline" onClick={() => handleSendReminder(p.id)}>
                    ...
                    <div className="p-4 border border-neutral-200 dark:border-neutral-800 rounded-xl">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{formatCurrency(borrowerRatingData.loanHistory?.totalBorrowed || 0, loan.currency)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl max-w-md w-full p-6 my-8">
            ...
            <div className="bg-neutral-50 dark:bg-neutral-800/50 rounded-xl p-4 mb-4">
              <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-1">Amount</p>
              <p className="text-2xl font-bold text-neutral-900 dark:text-white">{formatCurrency(loan.amount, loan.currency)}</p>
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    <div className="w-8 h-8 bg-neutral-900 dark:bg-white rounded mx-auto mb-1 flex items-center justify-center">
                      <CreditCard className="w-4 h-4 text-white dark:text-neutral-900" />
                    </div>
                  ...
                  >
                    <div className="w-8 h-8 bg-neutral-900 dark:bg-white rounded mx-auto mb-1 flex items-center justify-center">
                      <span className="text-white dark:text-neutral-900 font-bold">$</span>
                    </div>
                  ...
                  >
                    <div className="w-8 h-8 bg-neutral-900 dark:bg-white rounded mx-auto mb-1 flex items-center justify-center">
                      <span className="text-white dark:text-neutral-900 font-bold">V</span>
                    </div>
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={handleSendFunds} className="flex-1" disabled={fundsSending || !fundsProofFile}>
                {fundsSending ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2" />
                    Confirming...
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl max-w-md w-full p-6">
            ...
            <p className="text-neutral-600 dark:text-neutral-400 mb-4">
              Send a reminder email to <strong>{(loan?.borrower as any)?.full_name || 'the borrower'}</strong>.
            </p>
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                placeholder="e.g., Friendly reminder about your upcoming payment..."
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1" onClick={() => handleSendReminder()} disabled={sendingReminder}>
                {sendingReminder ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2" />
                    Sending...
      ...
      <ConfirmDialog
        ...
        title={paymentConfirmDialog.isBorrower ? 'Confirm Payment' : 'Process Payment'}
        message={
