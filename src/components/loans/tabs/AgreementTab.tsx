...
export function AgreementTab({ loan }: AgreementTabProps) {
  const borrowerSigned = (loan as any).borrower_signed;
  const lenderSigned = (loan as any).lender_signed;
  const borrowerSignedAt = (loan as any).borrower_signed_at;
  const lenderSignedAt = (loan as any).lender_signed_at;
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-neutral-500 dark:text-neutral-400">
                {borrowerSignedAt
                  ? `Signed on ${formatDate(borrowerSignedAt)}`
                  : 'Not signed yet'}
              </p>
            </div>
          </div>
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-neutral-500 dark:text-neutral-400">
                {lenderSignedAt ? `Signed on ${formatDate(lenderSignedAt)}` : 'Not signed yet'}
              </p>
            </div>
          </div>
        </div>
