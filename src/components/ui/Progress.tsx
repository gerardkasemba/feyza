...
interface ProgressProps {
  ...
  showLabel?: boolean;
  className?: string;
  /** Optional: override the bar color (e.g. `bg-emerald-500` for score-indicative UI) */
  indicatorClassName?: string;
}

export function Progress({ value, max = 100, size = 'md', showLabel = false, className, indicatorClassName }: ProgressProps) {
  ...
  return (
    <div className={cn('w-full', className)}>
      <div className={cn('w-full bg-neutral-100 dark:bg-neutral-700 rounded-full overflow-hidden', sizes[size])}>
        <div
          className={cn('h-full rounded-full transition-all duration-500 ease-out', indicatorClassName ?? defaultIndicator)}
